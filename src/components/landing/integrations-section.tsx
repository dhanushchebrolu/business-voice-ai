...
export function IntegrationsSection() {
  return (
    <section id="integrations" className="border-t border-slate-200 bg-blue-50/40 py-24 sm:py-32">
      <div className="mx-auto max-w-[1200px] px-5 sm:px-8">
        <div className="grid gap-14 lg:grid-cols-2 lg:items-center lg:gap-20">
          <div>
            ...
            <p className="mt-6 max-w-sm text-sm leading-relaxed text-slate-500 sm:text-base">
              No API keys to manage — connect an account and your AI employee starts using it.
            </p>
          </div>

          <div className="relative" aria-hidden="true">
            <div className="absolute -top-4 left-4 h-48 w-64 rotate-[-6deg] rounded-2xl border border-slate-200 bg-white shadow-[0_20px_40px_-20px_rgba(37,99,235,0.2)] sm:h-56 sm:w-72" />
            <div className="absolute -top-2 left-2 h-48 w-64 rotate-[-2deg] rounded-2xl border border-slate-200 bg-white shadow-[0_20px_40px_-20px_rgba(37,99,235,0.2)] sm:h-56 sm:w-72" />
            <div className="relative h-48 w-64 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_20px_40px_-20px_rgba(37,99,235,0.2)] sm:h-56 sm:w-72">
              <div className="grid grid-cols-4 gap-2">
                {CATEGORIES.map((label) => (
                  <div
                    key={label}
                    className="rounded-lg border border-blue-100 bg-blue-50 px-1.5 py-2 text-center text-[9px] font-medium leading-tight text-blue-700"
                  >
                    {label}
                  </div>
                ))}
            ...
            <div className="absolute -right-2 bottom-[-28px] w-52 rounded-2xl border border-slate-200 bg-white p-3.5 shadow-[0_24px_48px_-20px_rgba(37,99,235,0.25)] sm:-right-6 sm:bottom-[-32px] sm:w-60">
              <p className="text-xs font-semibold text-slate-900">Connected</p>
                ...
                {group.items.map((item) => (
                  ...
                  >
                    ...
                    {item.status === "connect" ? (
                      ...
                      <span className="rounded-full border border-slate-200 px-2.5 py-1 text-[10px] font-medium text-slate-400">
                        Coming soon
                      </span>
                    )}
                  </li>
