...
} from "./google-calendar-crypto.server.ts";

const ENV_KEY = "GOOGLE_CALENDAR_CREDENTIAL_ENCRYPTION_KEY";
...
describe("round trip", () => {
  test("decrypting an encrypted value returns the exact original plaintext", () => {
    const plaintext = JSON.stringify({ refreshToken: "1//0g_example_google_refresh_token" });
...
describe("ciphertext shape", () => {
  test("is versioned and dot-delimited: google_cal_cred.v1 prefix + 3 more parts (iv, tag, ciphertext)", () => {
    const stored = encryptCredential("secret-value");
    assert.equal(stored.startsWith("google_cal_cred.v1."), true);
    const remainder = stored.slice("google_cal_cred.v1.".length).split(".");
    assert.equal(remainder.length, 3);
  });

  test("never contains the plaintext as a literal substring", () => {
    const plaintext = "1//0g_super_secret_refresh_token_xyz";
    const stored = encryptCredential(plaintext);
    assert.doesNotMatch(stored, new RegExp(plaintext));
  });
});

const PREFIX = "google_cal_cred.v1";
...
describe("malformed ciphertext", () => {
  test("wrong version tag is rejected", () => {
    ...
    const { iv, tag, ciphertext } = splitStored(stored);
    const wrongVersion = ["google_cal_cred.v2", iv, tag, ciphertext].join(".");
    assert.throws(() => decryptCredential(wrongVersion), CredentialCryptoError);
  });

  test("wrong number of segments is rejected", () => {
    assert.throws(
      () => decryptCredential("google_cal_cred.v1.onlytwoparts"),
      CredentialCryptoError,
    );
  });
